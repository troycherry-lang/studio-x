...
# v3.1: Prompt weighting, Flux workflow, body preset support.

import random
from config import (
    DEFAULT_MODEL, DEFAULT_WIDTH, DEFAULT_HEIGHT, DEFAULT_STEPS, DEFAULT_CFG,
    DEFAULT_LORA_STRENGTH, DEFAULT_NEGATIVE, PORTRAIT_KEYWORDS, BODY_KEYWORDS,
    BODY_POSITIVE, BODY_NEGATIVE, BODY_PRESETS, PROMPT_WEIGHTS,
...
class Builder:
    ...
    def _apply_weights(self, prompt_text):
        ...
        for keyword, weight in PROMPT_WEIGHTS.items():
            # Replace standalone keyword with weighted version
            # Only weight if not already weighted
    ...
    def _prompt(self, task, user_text, anatomy=False, body_preset="default", weight_strength=1.0):
        ...
        if weight_strength != 1.0:
            # Wrap entire user prompt in weight modifier
    ...
    def _checkpoint(self, model, lora=None, lora_strength=DEFAULT_LORA_STRENGTH, flux=False):
        """Load checkpoint (SDXL) or Flux UNet + CLIP + VAE."""
        if flux:
            return self._flux_checkpoint(lora, lora_strength)
    ...
    def _flux_checkpoint(self, lora=None, lora_strength=DEFAULT_LORA_STRENGTH):
        """Load Flux: UNET + DualCLIP + VAE separately."""
        ...
        if lora and lora not in ("None", "", None):
            ...
            return vae, lora_node, clip

        return vae, model, clip

    # ── Task: Create (text-to-image) ─────────────────────────────────

    def create(self, prompt, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT, steps=DEFAULT_STEPS,
               cfg=DEFAULT_CFG, seed=-1, model=DEFAULT_MODEL, anatomy=False, lora=None,
               lora_strength=DEFAULT_LORA_STRENGTH, negative_prompt=None,
               body_preset="default", weight_strength=1.0, flux=False, flux_guidance=None):
        self.nodes = {}
        self.id_counter = 0
        if seed == -1:
        ...
        if flux:
            return self._flux_create(prompt, width, height, steps, seed, anatomy, lora,
                                     lora_strength, negative_prompt, body_preset, weight_strength, flux_guidance)
    ...
    def _flux_create(self, prompt, width, height, steps, seed, anatomy, lora,
                     lora_strength, negative_prompt, body_preset, weight_strength, flux_guidance):
        """Flux text-to-image workflow."""
        vae, mdl, clip = self._flux_checkpoint(lora, lora_strength)
    ...
    def face(self, prompt, ref_image, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT,
             ...
             anatomy=False, lora=None, lora_strength=DEFAULT_LORA_STRENGTH,
    ...
    def inpaint(self, prompt, image, mask, task="wardrobe", width=DEFAULT_WIDTH,
                ...
                lora=None, lora_strength=DEFAULT_LORA_STRENGTH,
                negative_prompt=None, body_preset="default", weight_strength=1.0):
        self.nodes = {}
        self.id_counter = 0
        if seed == -1:
    ...
    def refine(self, image, prompt="", denoise=0.5, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT,
               steps=DEFAULT_STEPS, cfg=DEFAULT_CFG, seed=-1, model=DEFAULT_MODEL,
               upscale=None, lora=None, lora_strength=DEFAULT_LORA_STRENGTH,
               negative_prompt=None, body_preset="default", weight_strength=1.0):
        self.nodes = {}
        self.id_counter = 0
        if seed == -1:
        ...
        pos = self._node("CLIPTextEncode", {
            "text": self._prompt("refine", prompt, False, body_preset, weight_strength) if prompt else "high quality, detailed",
            "clip": [mdl, 1],
        })
